...
FX_BOOL Document::mailForm(OBJ_METHOD_PARAMS)
{
	...
	CJS_Context* pContext = (CJS_Context*)cc;
	ASSERT(pContext != NULL);
	CPDFXFA_App* pEnv = pContext->GetReaderApp();
	ASSERT(pEnv != NULL);
	CJS_Runtime* pRuntime = pContext->GetJSRuntime();
	ASSERT(pRuntime != NULL);

	pRuntime->BeginBlock();
    pEnv->GetFormFillEnv()->JS_docmailForm(textBuf.GetBuffer(), textBuf.GetLength(), bUI, (FX_LPCWSTR)cTo, (FX_LPCWSTR)cSubject, (FX_LPCWSTR)cCc, (FX_LPCWSTR)cBcc, (FX_LPCWSTR)cMsg);
	pRuntime->EndBlock();
	return TRUE;
}
...
FX_BOOL Document::mailDoc(OBJ_METHOD_PARAMS)
{
	...
	pRuntime->BeginBlock();
	CPDFXFA_App* pEnv = pRuntime->GetReaderApp();
	pEnv->GetFormFillEnv()->JS_docmailForm(NULL, 0, bUI, (FX_LPCWSTR)cTo, (FX_LPCWSTR)cSubject, (FX_LPCWSTR)cCc, (FX_LPCWSTR)cBcc, (FX_LPCWSTR)cMsg);
	pRuntime->EndBlock();
