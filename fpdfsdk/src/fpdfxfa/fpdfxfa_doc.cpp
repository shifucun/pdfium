...
CPDFXFA_Document::~CPDFXFA_Document()
{
	...
	if (m_pJSContext)
	{
		m_pApp->GetJSRuntime()->ReleaseContext(m_pJSContext);
		m_pJSContext = NULL;
	}
...
void CPDFXFA_Document::GotoURL(XFA_HDOC hDoc, FX_WSTR bsURL, FX_BOOL bAppend)
{
	...
	pEnv->FFI_GotoURL(this, bsURL, bAppend);
...
FX_BOOL	CPDFXFA_Document::SetGlobalProperty(XFA_HDOC hDoc, FX_BSTR szPropName, FXJSE_HVALUE hValue)
{
	if (hDoc != m_pXFADoc)
		return FALSE;
	if (!m_pApp || !m_pApp->GetJSRuntime())
		return FALSE;
	return m_pApp->GetJSRuntime()->SetHValueByName(szPropName, hValue);
}
FX_BOOL	CPDFXFA_Document::GetPDFScriptObject(XFA_HDOC hDoc, FX_BSTR utf8Name, FXJSE_HVALUE hValue)
{
	...
	if (!m_pJSContext)
	{
		m_pApp->GetJSRuntime()->SetReaderDocument(m_pSDKDoc);
		m_pJSContext = m_pApp->GetJSRuntime()->NewContext();
	}

	return _GetHValueByName(utf8Name, hValue, m_pApp->GetJSRuntime());
...
FX_BOOL CPDFXFA_Document::GetGlobalProperty(XFA_HDOC hDoc, FX_BSTR szPropName, FXJSE_HVALUE hValue)
{
	...
	return _GetHValueByName(szPropName, hValue, m_pApp->GetJSRuntime());
