...
IFXJS_Runtime* CPDFDoc_Environment::GetJSRuntime()
{
	if(!IsJSInitiated())
		return NULL;
	return	m_pJSRuntime = m_pDoc->GetApp()->GetJSRuntime();
}
...
void CPDFSDK_PageView::LoadFXAnnots()
{
	...
	if (m_pSDKDoc->GetDocument()->GetDocType() == DOCTYPE_DYNIMIC_XFA)
	{
		...
		IXFA_WidgetIterator* pWidgetHander = pageView->CreateWidgetIterator(XFA_TRAVERSEWAY_Form, XFA_WIDGETFILTER_Viewable|XFA_WIDGETFILTER_AllType);
		if (!pWidgetHander)
			return;
