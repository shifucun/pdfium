...
typedef struct  _FPDF_FORMFILLINFO
{
	...
	**/
	...
	/** 
	* Method: FFI_GetFilePath
	*			This method will get the current opened file path.
	* Interface Version:
	*			1
	* Implementation Required: 
	*			yes
	...
	*		filePath		-	Pointer to the data buffer to receive the file path.Can be NULL, in UTF-16LE format.
	*		length			-   The length of the buffer, number of bytes. Can be 0.
	* Return value:
	* 		Byte size of the data buffer.
	**/
	int (*FFI_GetFilePath)(struct _FPDF_FORMFILLINFO* pThis, FPDF_FILEHANDLER* pFileHandler, void* filePath, int length);
	/** 
	* Method: FFI_EmailTo
