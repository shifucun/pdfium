...
FX_BOOL CXFA_WidgetAcc::ProcessFormatTestValidate(CXFA_Validate validate, FX_BOOL bVersionFlag)
{
...
void CXFA_WidgetAcc::ProcessNullTestValidate(CXFA_Validate validate, FX_INT32 iFlags, FX_BOOL bVersionFlag)
{
    if (bVersionFlag) {
        return;
    }
...
FX_INT32 CXFA_WidgetAcc::ProcessValidate(FX_INT32 iFlags)
{
    ...
    if (script) {
        ...
        iRet = ExecuteScript(script, &eParam, (bInitDoc || bStatus) ? NULL : &pRetValue);
    }
    ...
    if (bInitDoc) {
        ...
        bFormat = ProcessFormatTestValidate(validate, bVersionFlag);
        if (!bVersionFlag) {
            bVersionFlag = GetDoc()->GetXFADoc()->HasFlag(XFA_DOCFLAG_Scripting);
        }
        ProcessNullTestValidate(validate, iFlags, bVersionFlag);
    }
    if (!bFormat) {
