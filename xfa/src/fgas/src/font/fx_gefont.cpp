...
FX_BOOL CFX_GEFont::LoadFont(CFX_Font *pExtFont)
{
    if (m_pFont != NULL) {
        return FALSE;
    }
...
FX_INT32 CFX_GEFont::GetGlyphIndex(FX_WCHAR wUnicode, FX_BOOL bRecursive, IFX_Font **ppFont, FX_BOOL bCharCode)
{
    ...
    IFX_Font *pFont = NULL;
    m_FontMapper.Lookup((void*)(FX_UINTPTR)wBitField, (void*&)pFont);
    ...
    if (m_pFontMgr != NULL && bRecursive) {
        ...
        if (pFont != NULL) {
            ...
            m_FontMapper.SetAt((void*)(FX_UINTPTR)wBitField, (void*)pFont);
