...
static FX_LPCSTR szCompatibleModeScript_ConstructorExtraMethods =
    "(function (global, list) {\n\
		...
		for (objname in list) {\n\
			...
			if (globalobj) {\n\
				list[objname].forEach(\n\
					function (name) {\n\
						if (!globalobj[name]) {\n\
							globalobj[name] = function (obj) {\n\
...
{
    if (dwCompatibleFlags & FXJSE_COMPATIBLEMODEFLAG_CONSTRUCTOREXTRAMETHODS) {
        FXJSE_ExecuteScript(hContext, szCompatibleModeScript_ConstructorExtraMethods, NULL, NULL);
    }
}
